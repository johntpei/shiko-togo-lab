  ...
  await withReviewEnv(async () => {
    ...
    const result = await runIntegratedReview(twoSessions, "統合レビュー — テスト", {
      ...
      save: (input) => {
        ...
        assert.equal(
          input.payload.commonThemes[0]?.evidence[0]?.evidenceRef,
          "S01:M001:E01",
        );
        assert.equal(
          input.payload.shifts[0]?.beforeEvidence[0]?.evidenceRef,
          "S01:M001:E01",
        );
        assert.equal(
          input.payload.shifts[0]?.afterEvidence[0]?.evidenceRef,
          "S02:M001:E01",
        );
        assert.equal(
          input.payload.tensions[0]?.sideA?.evidence[0]?.evidenceRef,
          "S01:M001:E01",
        );
        assert.equal(
          input.payload.tensions[0]?.sideB?.evidence[0]?.evidenceRef,
          "S02:M001:E01",
        );
        assert.equal(
          input.payload.crossInsights[0]?.evidence[0]?.evidenceRef,
          "S01:M001:E01",
        );
