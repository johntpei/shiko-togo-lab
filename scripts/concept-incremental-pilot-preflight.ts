import {
  ...
  CONCEPT_INCREMENTAL_PILOT_PREFLIGHT_HELP,
  defaultOpenReadonlyIncrementalPilotDb,
  runConceptIncrementalPilotPreflight,
} from "@/lib/concepts/incremental/pilot-preflight";
import {
  countConceptAliases,
...
function main() {
  ...
  if (argv.includes("--help") || argv.includes("-h")) {
    console.log(CONCEPT_INCREMENTAL_PILOT_PREFLIGHT_HELP);
    process.exit(0);
  }
  ...
  const result = runConceptIncrementalPilotPreflight(argv, {
    openDb: () => db,
    dbPath,
  });
  ...
  if (
    ...
    console.error("preflight changed DB counts");
    process.exit(1);
  }
  process.exit(result.report.status === "ready" ? 0 : 1);
}
