...
import {
  ...
  CONCEPT_INCREMENTAL_CAPTURE_HELP,
...
} from "@/lib/concepts/incremental/capture-intent";
...
async function main() {
  ...
  if (argv.includes("--apply")) {
    console.error(CONCEPT_INCREMENTAL_CAPTURE_APPLY_ERROR);
    process.exit(1);
  }
  if (argv.includes("--help") || argv.includes("-h")) {
    console.log(CONCEPT_INCREMENTAL_CAPTURE_HELP);
    process.exit(0);
  }

  const parsed = parseConceptIncrementalCaptureArgs(argv);
  if (parsed.malformed || !parsed.sessionId) {
    console.error(CONCEPT_INCREMENTAL_CAPTURE_HELP);
    process.exit(1);
  }

  if (existsSync(resolve(parsed.intentPath))) {
    console.log(REAL_FROZEN_EXISTING_MATCH_INTENT_BLOCKED);
  ...
  const result = await runConceptIncrementalCaptureIntent(argv, {
    openDb: () => db,
  ...
  if (
    ...
    console.error("intent capture changed DB counts");
    process.exit(1);
  }
  process.exit(
    result.report.classification === REAL_FROZEN_EXISTING_MATCH_INTENT_READY ||
    result.report.classification === REAL_FROZEN_EXISTING_MATCH_NO_MATCHES
      ? 0
      : 1,
  );
}
