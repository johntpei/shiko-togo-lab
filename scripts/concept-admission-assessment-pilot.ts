...
import {
  ...
  CONCEPT_ASSESSMENT_PILOT_HELP,
  formatConceptAssessmentPilotSummary,
  runConceptAssessmentPilot,
  type AdmissionEvidenceSession,
} from "@/lib/concepts/admission/assessment-pilot";
...
async function main() {
  ...
  if (argv.includes("--apply")) {
    console.error(CONCEPT_ASSESSMENT_PILOT_APPLY_ERROR);
    process.exit(1);
  }
  ...
  const result = await runConceptAssessmentPilot(argv, {
  ...
  if (!result.ok) {
    console.error(
      result.error === CONCEPT_ASSESSMENT_PILOT_HELP
        ? CONCEPT_ASSESSMENT_PILOT_HELP
        : result.error,
    );
    process.exit(1);
  ...
  console.log(formatConceptAssessmentPilotSummary(result.report));
}
